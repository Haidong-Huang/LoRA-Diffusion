...
def safe_device(x, device="cpu"):
    if device == "cpu":
        ...
        if torch.cuda.is_available():
            # Disable cuDNN before moving RNN modules to GPU to avoid compatibility issues
            # with newer GPU architectures (e.g., RTX 5090 with sm_120)
            cudnn_was_enabled = torch.backends.cudnn.enabled
            torch.backends.cudnn.enabled = False
            try:
                result = x.to(device)
            finally:
                # Re-enable cuDNN for other operations (conv, etc.)
...
def get_task_embs(cfg, descriptions):
    ...
    if cfg.task_embedding_format == "bert" or cfg.task_embedding_format == "one-hot":
        import os
        # Prefer user-provided local model under ./bert-model
        bert_local_path = to_absolute_path("./bert-model")
        bert_cache_dir = to_absolute_path("./bert")
        ...
        if os.path.exists(bert_local_path):
            ...
            has_config = all(
                os.path.exists(os.path.join(bert_local_path, f))
            ...
            if has_config and has_model:
                local_model_path = bert_local_path
                print(f"[info] Using local BERT model from: {local_model_path}")

        # Fallback to old ./bert/bert-base-cased layout if present
        if local_model_path is None:
            legacy_local = to_absolute_path("./bert/bert-base-cased")
            if os.path.exists(legacy_local):
                required_files = ["config.json", "tokenizer_config.json", "vocab.txt"]
                model_files = ["pytorch_model.bin", "model.safetensors"]
                has_config = all(
                    os.path.exists(os.path.join(legacy_local, f))
                    for f in required_files
                )
                has_model = any(
                    os.path.exists(os.path.join(legacy_local, f))
                ...
                if has_config and has_model:
                    local_model_path = legacy_local
                    print(f"[info] Using local BERT model from: {local_model_path}")

        if local_model_path:
            tz = AutoTokenizer.from_pretrained(local_model_path, local_files_only=True)
            model = AutoModel.from_pretrained(local_model_path, local_files_only=True)
        else:
            # Last resort: use cache_dir (may try to download if not fully offline)
            tz = AutoTokenizer.from_pretrained(
                "bert-base-cased", cache_dir=bert_cache_dir
            )
